...
{	
	...
	if [[ $3 = "0" ]]; then
		echo "$ky$error$tipo not exported\033[m"
	else
		echo "$ky$success$tipo exported \033[m in $PATH_TO_EXPORT"
	fi
}
...
do	
	...
	if [[ $c != "#" ]]; then
		...
		echo "$i$ky\c"
		#f=$PATH_TO_EXPORT/$i.xml		
		icommand -cmd export -name $PATH_TIPO/$i -type report -file $PATH_TO_EXPORT/$i.xml >/dev/null;
