...
<xsl:template match="REOffice">
  <tr>
   <td><!-- NRDSMemberID is not the office ID but I can't find anything else to use for office ID in the spec -->
    <a><xsl:attribute name="href">searchAgentAction.jsp?searchQuery=%28ListOfficeOfficeID%3D<xsl:value-of select="OfficeID"/>%29&amp;queryType=DMQL&amp;searchStandardNames=1</xsl:attribute><xsl:value-of select="Name"/></a>
   </td>
    <td>
