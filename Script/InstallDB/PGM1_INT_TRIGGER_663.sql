create or replace
TRIGGER trg_consistencybam BEFORE
  INSERT OR
  UPDATE ON "_TemplateSRR_DO" REFERENCING OLD AS OLD NEW AS NEW FOR EACH ROW
DECLARE
  newExecutionTime FLOAT;
  newFixedTime    FLOAT;
  stato           VARCHAR(1);
  BEGIN
    newFixedTime    :=0;
    ...
    IF INSERTING THEN
      ...
      into newExecutionTime
      from DUAL;
      :new."_ExecutionTime" := newExecutionTime;
    ELSIF UPDATING THEN
        select
          NVL((extract(hour from :new."_LastUpdate"- :new."_CreationDate"))*3600+
          (extract(minute from :new."_LastUpdate"- :new."_CreationDate"))*60+
          ...
          (extract(second from :new."_ErrorFD"- :new."_ErrorCD")),null) FT
        into newExecutionTime, newFixedTime
        from DUAL;
        :new."_ExecutionTime"     := newExecutionTime;
        :new."_FixedTime"         := newFixedTime;
      ...
      IF ((:old."_Status"  ='Successful') AND (:new."_Status" ='Running' OR :new."_Status" ='Started')) -- BLOCK
        ...
        */
        ...
        :new."_ExecutionTime"     := :old."_ExecutionTime";
        :new."_FixedTime"         := :old."_FixedTime";
      END IF;
